...
| `cka` | 358장 | `pages/` 21개 파일로 분할. Kubernetes CKA 시험 대비. mermaid 13개 |
...
- **`#` = 장, `##` = 본문 슬라이드. 이 둘이 목차 트리의 두 단계다.**
  장 구분 슬라이드는 `#` + `layout: section`, 본문 슬라이드 제목은 `##`.
  `###` 이하는 목차에 안 잡힌다. **`#`을 장 구분 외의 용도로 쓰면 목차 트리가 어긋난다.**
...
덱의 장 구성과 배점은 **CKA 커리큘럼 v1.35**를, 명령·API 버전은 **시험 환경 Kubernetes v1.35**를
...
덱의 배점·도메인·버전 표기를 고칠 때는 반드시 위 출처를 다시 조회할 것.
...
**"이 기능은 없다"는 판단은 kubernetes.io/docs를 직접 조회한 뒤에 내릴 것.**

### 유지할 서술 원칙

- 덱 전체의 축은 **"선언된 상태(spec)와 실제 상태(status)의 차이를 줄이는 루프"** — 0장과 20장이 이걸 감싼다
- 각 장이 **"실제로 무엇이 일어나는가"** → **명령** → **함정** 순서로 간다
- **`.exam-tip`(시험 포인트)과 `.pitfall`(함정)** 두 박스가 이 덱 고유 장치다. 클래스는 덱 `style.css`에 있다
- 배점 순서와 학습 순서를 구분한다 — 트러블슈팅(30%)이 18장인 건 앞의 전부가 재료라서다
- 장 끝마다 요약 슬라이드. 19장은 치트시트, 20장은 장별 한 줄 요약 — **시험 직전에 이 둘만 봐도 되게** 유지한다

### 밀도 관련 실측 메모

이 덱은 **YAML/명령 블록이 지배하는 슬라이드가 많다.** 그래서 CLAUDE.md 상단의 경고가 그대로 적용된다.

- 전수 검사 첫 회에 **57장이 넘쳤고**, `dense`/`denser`로 해결된 건 40장이었다
- **남은 17장은 전부 코드 지배형**이라 클래스로는 안 줄었다. YAML을 **flow 스타일**로 압축해서 줄 수를 줄였다
  (`matchLabels:` 두 줄 → `matchLabels: { app: db }` 한 줄). 이 방식이 폰트를 더 줄이는 것보다 낫다
- 그래도 안 되면 두 장으로 나눈다 (16장 "오버레이 작성"이 그렇게 쪼갠 사례)
- 실측 기준: `denser`에서 코드 **1줄 ≈ 13.5px**, `dense`에서 **1줄 ≈ 17px**
