...
addons/deck-nav/          # 모든 덱이 공유하는 내비게이션 (로컬 Slidev 애드온)
...
4. 공통 내비게이션 애드온을 붙인다 — 덱 `package.json`에 아래 두 항목.
   목차/홈 버튼과 터치 기기 조작 레이어가 여기서 온다 ([addons/deck-nav](addons/deck-nav)).

   ```json
   "slidev": { "addons": ["@decks/addon-nav"] },
   "devDependencies": { "@decks/addon-nav": "workspace:*" }
   ```

   덱 고유 강조 색이 필요하면 덱 `style.css`에 `:root { --deck-accent: … }`를 둔다.
...
덱 안의 하단 내비게이션 목차(공통 애드온)도 같은 기준으로 트리를 만들고,
열 때 **현재 슬라이드가 속한 장만 펼쳐서** 보여준다.

## 모바일에서 보기

터치 기기(`hover: none`)에서는 Slidev 기본 내비게이션 바 대신 전용 조작 레이어가 뜬다.
기본 바는 hover에 기대고 있어서, 안 보이는 상태로 화면 좌하단의 탭을 가로채는 문제가 있다.

- **화면 좌우 가장자리(각 22%) 탭** → 이전/다음. 스와이프도 그대로 동작한다.
  링크·버튼·캔버스 위에서는 넘어가지 않으므로 실물 UI 데모나 Phaser 데모 조작과 겹치지 않는다.
- **오른쪽 아래 쪽수 알약** 탭 → 목차 / 홈 / 라이트·다크 / 전체화면 / 이전·다음 바가 펼쳐진다
  (6초 뒤 자동으로 접힘).
- 데스크톱(마우스)에서는 기존 hover 내비게이션 바 그대로다.
