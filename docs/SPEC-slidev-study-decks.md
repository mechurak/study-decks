...
### 4.6 `decks/supabase/slides.md` (샘플)

구조 검증용 최소 덱. 5~6장 정도면 충분하다. 다음 기능이 실제로 렌더되는지 확인할 것:

- 프론트매터에 `title`, `description` (인덱스 생성 검증용) + `routerMode: hash` (필수, 4.5 참고)
- Mermaid 다이어그램 1개 (Auth 시퀀스 다이어그램)
- 단계별 코드 하이라이트 — 실제 구현은 `{1-3|5-8|all}` (RLS 정책 SQL)
- `v-click` 점진적 노출
